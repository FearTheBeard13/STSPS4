...
act_labels<-read.csv("C:/STSPS4/activity_labels.txt",header=FALSE, dec=".", sep=" ")
colnames(act_labels)<-c("n","activity")

test_lines<-readLines(file("C:/STSPS4/test/X_test.txt", open="r"))
train_lines<-readLines(file("C:/STSPS4/train/X_train.txt", open="r"))
data_lines<-c(test_lines,train_lines)

test_act_numb<-readLines(file("C:/STSPS4/test/Y_test.txt", open="r"))
train_act_numb<-readLines(file("C:/STSPS4/train/Y_train.txt", open="r"))
